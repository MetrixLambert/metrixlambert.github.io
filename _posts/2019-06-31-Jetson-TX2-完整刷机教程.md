...
[![](https://github.com/MetrixLambert/uploadpicture/raw/master/2019_06_01_jetson_tx2.png)](http://metrixlambert.github.io/)
...
[![](https://github.com/MetrixLambert/uploadpicture/blob/master/2019_06_01_nvidia_family.png?raw=true)]((http://metrixlambert.github.io/))
...
**Flash OS** 是指给TX2重新刷系统的过程。虽然 TX2 出厂时都会自带系统，但是为了和 Jetpack 里面其他软件包的版本配套，我们还是选择重新刷系统（ **Reflash OS** ）。
...
检查 USB 端口是否有 TX2 连接。如果出现了 Nvidia Corp 的字样，那么说明强制恢复模式启动成功。
...
这次底层环境配置的过程真的可谓是踩坑无数。关键还大坑不少，小坑不断。由于篇幅所限，在这里只将一些比较大的，网上不好找到解决方式的坑分享出来。



### 1.最大的坑：非要连接路由器，不能连接 WIFI ？
...
3000多字一个一个手码上去，更不用说需要重新收集自己踩坑历程中浏览过的那些关键网站，遇到过的关键步骤。

但是个人认为，合理的记录自己的成长历程，并及时与其他人分享，不仅有利于开发者本人的迅速成长，还能够形成圈子里面的良性循环，你帮我，我帮你，共同建立一个开放，分享的环境。

博主并不介意其他人转载这篇放在Github上的文章，但是麻烦请署一下作者名称。
